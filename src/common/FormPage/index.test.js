...
describe("FormPage", () => {
  ...
  function buildFormChild(
    props,
    mutate = () => null,
    formState = {},
    response
  ) {
    const form = findDataTest(
      buildMutationChild(props, mutate, response),
      "form"
    )
  ...
  it("passes the right props to the Mutation component", () => {
    ...
    expectDataTest(wrapper, "mutation").toHaveProp({
  ...
  it("passes the initial values to the Form component", () => {
    ...
    expectDataTest(wrapper, "form").toHaveProp(
      "initialValues",
      "SOME INITIALVALUES"
    )
  })
  ...
  describe("onSubmit handler", () => {
    ...
    it("returns null if the mutation is resolved", async () => {
      const wrapper = buildMutationChild({ parseVariables: v => v }, () =>
        Promise.resolve()
      )
  ...
  describe("render prop", () => {
    it("shows a snackbar and redirects on success", () => {
      ...
      expectDataTest(wrapper, "success-message").toHaveProp(
        "message",
        "SOME SUCCESSMESSAGE"
      )
    ...
    it("shows a snackbar on error", () => {
      ...
      expectDataTest(wrapper, "error-message").toHaveProp(
        "message",
        "SOME ERRORMESSAGE"
      )
    })
    ...
    it("renders the page header with the right title", () => {
      const wrapper = buildFormChild()
      expectDataTest(wrapper, "page-header").toHaveProp("title", "SOME TITLE")
    })
    ...
    describe("renders the page header buttons", () => {
      function runTest(submitting) {
        ...
        const renderedButtons = findDataTest(wrapper, "page-header").prop(
          "buttons"
        )
